...
class Kol extends StatefulWidget {
  ...
  @override
  _KolState createState() => _KolState();
}
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          if (_formKey.currentState!.validate()) {
            int sayi = Hive.box("Kol").length + 1;
            setState(() {
              var gun = DateTime.now().day;
              var ay = DateTime.now().month;
              var yil = DateTime.now().year;
              if (_textFieldController.text != "") {
                Hive.box("Kol").put("$gun.$ay.$yil tarihli $sayi. Ölçüm",
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return ListView.builder(
            itemCount: Hive.box("Kol").length,
            itemBuilder: (context, index) {
              var key = Hive.box("Kol").keyAt(index);
              var value = Hive.box("Kol").get(key);
              return ListTile(
